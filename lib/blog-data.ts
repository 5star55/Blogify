...
export async function getPosts(): Promise<Post[]> {
  const res = await fetch('https://social-media-app-xhv2.onrender.com/api/posts');
...
export async function getSpecificPost(id:string){
  const res = await fetch(`https://social-media-app-xhv2.onrender.com/api/posts/${id}`);
  if (!res.ok) throw new Error(`Failed: ${res.status}`);
...
export async function deletePost(id: string): Promise<void> {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Failed to delete post: ${res.status} ${message}`);
  }
}
...
export async function updatePost(id: string, payload: UpdatePostPayload): Promise<Post> {
  ...
  if (!res.ok) {
    const message = await res.text();
    throw new Error(`Failed to update post: ${res.status} ${message}`);
  }
  const json = await res.json();
